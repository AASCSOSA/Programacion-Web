...
@Query(value = "SELECT f.id_fertilizacion,f.cantidad_aplicacion,f.fecha_aplicacion,f.id_fertilizante,f.id_rancho, r.nombre_rancho FROM fertilizacion f JOIN fertilizante fe ON fe.id_fertilizante=f.id_fertilizante JOIN rancho r ON r.id_rancho=f.id_rancho WHERE YEAR(f.fecha_aplicacion) = YEAR(CURRENT_DATE) AND MONTH(f.fecha_aplicacion) = MONTH(CURRENT_DATE)", nativeQuery = true) 
